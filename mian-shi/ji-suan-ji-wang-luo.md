...
* 200 请求成功
* 204 请求成功，但是没有body数据返回
* 206 请求成功，返回部分body数据

3XX  主要代表了重定向

* 301 永久重定向，请求的资源永久移除
* 302 临时重定向，请求的资源还在，但需要访问另一个URL

301和302都会在响应头里面添加location字段，知名需要跳转的URL
...
* 404 Not Founding 资源未找到

5XX 主要代表了服务端的错误

* 500 笼统的内部错误
* 501 功能暂不可用，敬请期待
* 502 Bad Gateway 服务器作为网关异常
* 503 Service unvalable 服务器忙暂时无法响应
...
第二个方面，从本质来说，GET是安全且幂等的，即每一次的请求都具有相同的副作用，且不对服务器上的内容进行修改，而POST则不是，因此在需要增删改的场景使用POST是非常危险的。

## 21.域名解析之后，我们如何才能知道要访问什么？
